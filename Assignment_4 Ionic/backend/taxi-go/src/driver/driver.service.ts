...
export class DriverService {
  ...
  async create(createDriverDto: CreateDriverDto) :Promise<Object> {
    try {
       ...
       const hashedPassword = await bcrypt.hash(createDriverDto.driverPassword, saltOrRounds);
      const driver = await new this.DriverModel({...createDriverDto, driverPassword: hashedPassword, userRoles: ["driver"]}).save();
      ...
      if (error.errorResponse.keyValue && error.errorResponse.keyValue.driverEmail) {
        throw new HttpException({msg: 'Driver email is already registered.', error, status: "failed"}, HttpStatus.BAD_REQUEST);
      } else if (error.errorResponse.keyValue && error.errorResponse.keyValue.driverPhone) {
        throw new HttpException({msg: 'Driver phone number is already registered.', error, status: "failed"}, HttpStatus.BAD_REQUEST);
      } else {
  ...
  async findOne(email: String) : Promise<driverDocument> {
    try {
      const driver = await this.DriverModel.findOne({driverEmail: email});
      if (!driver) {

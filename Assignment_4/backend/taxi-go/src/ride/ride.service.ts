...
export class RideService {
  ...
  async findCurrentRideList() : Promise <Object> {
    try {
      const rideList = await this.rideModel.find({ rideStatus: { $in: [1, 2]}});
      if (!rideList) {
        throw new NotFoundException({msg: "Ride list is empty.", status: "failed"});
      }
      return {rideList, msg: "Ride list found", status: "success"};
  ...
  async findRideHistory() : Promise <Object> {
    try {
      const rideList = await this.rideModel.find({ rideStatus: { $in: [0, 3]}});
      if (!rideList) {
        throw new NotFoundException({msg: "Ride history list is empty.", status: "failed"});
      }
      return {rideList, msg: "Ride list found", status: "success"};
    } catch (error) {
      if (error.response && error instanceof NotFoundException) {
